...
def plot_stim_traces(s, cells, gap=20, mask_artifact=True,
                     baseline_subtract=False, trials='all'):
    """Concatenated trial-averaged cyc traces (mean +/- SEM) for each cell.

    Always plots the full cyc window (as generated by `rebuild_cyc`, or as
    loaded) — no cropping.
    ...
    if trials == 'sham':
        if s.has_photostim:
            from .photostim import cyc_trial_group, sham_target_numbers
            cand = np.isin(cyc_trial_group(s), sham_target_numbers(s))
            if cand.any():
                sel, mode = cand, 'sham'
            else:
